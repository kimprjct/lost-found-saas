...
@section('content')
    <div class="bg-white rounded-lg shadow p-6 max-w-xl">
        <form method="POST" action="{{ route('tenant.user-management.update', $user) }}" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-sm text-gray-700 mb-1">User ID</label>
                <input name="user_id" class="w-full border rounded px-3 py-2" value="{{ old('user_id', $user->user_id) }}" required />
            ...
            <div>
                ...
                <select name="department" class="w-full border rounded px-3 py-2" required>
                    ...
                    <option value="COT" {{ old('department', $user->department) == 'COT' ? 'selected' : '' }}>COT</option>
                    <option value="CEIT" {{ old('department', $user->department) == 'CEIT' ? 'selected' : '' }}>CEIT</option>
                    <option value="CAS" {{ old('department', $user->department) == 'CAS' ? 'selected' : '' }}>CAS</option>
            ...
            <div>
                ...
                <input name="email" type="email" class="w-full border rounded px-3 py-2" value="{{ old('email', $user->email) }}" required />
                @error('email')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
            </div>
            <div>
                <label class="block text-sm text-gray-700 mb-1">Contact Number</label>
                <input name="contact_number" class="w-full border rounded px-3 py-2" value="{{ old('contact_number', $user->contact_number) }}" required />
                @error('contact_number')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
            </div>
            <div>
                <label class="block text-sm text-gray-700 mb-1">New Password (leave blank to keep current)</label>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button class="px-4 py-2 rounded bg-blue-600 text-white">Update User</button>
            </div>
        </form>
    </div>
@endsection
